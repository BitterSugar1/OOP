...
ReportLab::ReportLab(const char* discipline, int totalWorks, double executionTime, const char* executor)
    : LaboratoryWorks(discipline, totalWorks, executionTime, executor),
    reportsSubmitted(0) {
    reports = new bool[totalWorks];
    for (int i = 0; i < totalWorks; ++i) {
        reports[i] = false;
